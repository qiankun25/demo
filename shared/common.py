...
class RabbitConfig:
    ...
    # 命令队列 TTL（毫秒）。
    # 默认值从 60s 提高到 15min，避免 fan-out 时消息在队列里等待过久被 DLQ，导致编排任务“永远 pending”。
...
class MockStorage:
    """
    Claim Check 存储：
    - 之前是进程内 dict（易丢失）；现在改为写入 MinIO（S3 兼容）。
    - 仍保持 async 接口；内部用 asyncio.to_thread 执行阻塞的 minio-py。
    ...
    def _unwrap(cls, raw: bytes) -> Any:
        if not raw.startswith(cls._MAGIC):
            # 兼容旧/外部写入：默认按 json 尝试，不行就返回 bytes
            try:
    ...
    async def list_keys(cls, prefix: str = "") -> List[str]:
        """列出（去掉 NEXUS_STORAGE_PREFIX 后的）key。"""
        cls._init()
        want = str(prefix or "")
